...
export function SiteHeader() {
  ...
  <Link href="/" className="flex-shrink-0">
    ...
    >
      <g fill={isMenuOpen ? "background" : "background"} className="transition-all duration-700">
            ...
            <ul className="flex flex-col gap-0">
              {navLinks.map((link, index) => (
                <li key={link.href} className="overflow-hidden">
                  <Link
                    href={link.href}
                    className={`block text-[70px] md:text-[89px] lg:text-[101px] tracking-tight font-regular leading-[0.95] transition-colors duration-100 ease-in-out ${isActive(link.href) ? "text-background" : "text-background/20 hover:text-background"}`}
                    style={{
